...
local f = e:WaitForChild("Storage").Remotes.CheckChildExists
...
game.DescendantAdded:Connect(function(k)
	...
	for _, n in ipairs(m) do
		if n.Name == "ReplicatedStorage" then
			e.Storage.Remotes.AntiCheat:FireServer("???", "using exploit.")
			return
		end
	end

	local o = k:FindFirstChild("Key")
	local p = e.Storage.Remotes.GetKey:InvokeServer()

	if o and l then
		if o.Value ~= p then
			e.Storage.Remotes.AntiCheat:FireServer(k.Name, "adding instance with wrong key - exploit.")
		end
	...
	elseif not o and not l then
		e.Storage.Remotes.AntiCheat:FireServer(k.Name, "adding instance with exploit.")
	end
end)
